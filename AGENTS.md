...
- [ ] The format check passes, and so do the gates: `python3 Scripts/CheckProjectFiles.py`, `Scripts/CheckDeterminism.py`, `Scripts/CheckDesign.py`, `Scripts/CheckSpelling.py`, `Scripts/CheckHudGeometry.py`. They are seconds and they are what CI runs; `CheckSpelling.py --fix` rewrites what it finds.
