...
**All six libraries hold real code.** ([`Design/Plan/`](Design/Plan/README.md): M0 complete, M1 built as far as its interface.) `NeuronCore` has the byte codec, the packet header, fixed point, the vector, the sine table and the PRNG; `NeuronServer` the Winsock endpoint and the tick schedule; `NeuronClient` the `DatagramSocket` endpoint, the packet queue, the window metrics and fit transforms, the Direct3D 12 device and swap chain, the scene target and the scaled present, the interface pass, the gesture seam that is the only way in, the CMO reader and the instanced mesh pass; `GameCore` the entity, the component catalog, the design table and the derived-stat function, the wire records, the command codec, the join and the starting layout; `GameLogic` the world, the tick, the state hash, command intake, ring slot assignment, the build system, the session table and the host loop; `GameClient` the replica store and interpolation clock, the camera and its gesture, the tap resolver, selection and group selection, the hull meshes and the order markers.
