...
**Five of the six libraries hold real code, and the sixth is the one that matters here.** ([`Design/Plan/M0-the-wire.md`](Design/Plan/M0-the-wire.md), M0.1 to M0.14.) `NeuronCore` has the byte codec, the packet header, fixed point, the vector, the sine table and the PRNG; `NeuronServer` the Winsock endpoint and the tick schedule; `NeuronClient` the `DatagramSocket` endpoint, the packet queue, the window metrics and fit transforms, and the Direct3D 12 device and swap chain; `GameCore` the entity, the wire records and the command codec; `GameLogic` the world, the tick, the state hash, command intake and the host loop. **`GameClient` is the one still holding a function that returns its own name**, so that every edge of the build — every reference, every include path, every link — is exercised by something rather than asserted. Its master include says to delete that placeholder when the library's first real declaration lands, and that is now the only one outstanding.
