...
**The tree is a shell.** Six libraries, two executables and six test suites exist; between them they hold a handful of functions that return their own name, so that every edge of the build — every reference, every include path, every link — is exercised by something rather than asserted. There is no game in it yet. Nothing below is a target to migrate towards: it describes the code as it must be written from the first real line.
...
**One departure from "every pass" is recorded and is not a violation.** The *interface* draws after the scale, straight into the back buffer at physical resolution, laid out in authored coordinates through the same fit transform this rule already computes once — so no pass branches on the window size and no layout number becomes conditional. The world still draws into the scene target. See [`Design/ADR/ADR-011`](Design/ADR/ADR-011-the-interface-draws-after-the-scale.md), which states why the intent of this rule survives its letter being broken, and what would put the interface back into the scene target.
...
**A correction, because this rule used to say something untrue.** It previously argued that `/arch:AVX2` lets MSVC contract `a*b+c` into an FMA under `/fp:precise`, and rested the no-floats requirement on that. **It does not, on this toolset.** Microsoft's [`/fp` documentation](https://learn.microsoft.com/cpp/build/reference/fp-specify-floating-point-behavior) states plainly: *"Floating-point contractions aren't generated by default under `/fp:precise`. This behavior is new in Visual Studio 2022."* The option table gives `/fp:precise` → `fp_contract(off)`, with contraction-on documented only for compilers **older** than VS2022. This tree pins `v145` (§3). A rule defended by a citation that does not survive being looked up is a rule somebody will relax for the wrong reason, so the argument is replaced rather than quietly dropped.

**The simulation still holds no floats, for three reasons that are true:**
...
Integers are reached by none of the three. Floats live in the renderer, where nothing is replayed.
