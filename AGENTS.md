...
**The tree is half a shell, and which half matters.** Six libraries, two executables and six test suites exist. **`NeuronCore`, `NeuronClient` and `NeuronServer` now hold real code** — the byte codec, the packet header and the two socket endpoints, each with a suite over it ([`Design/Plan/M0-the-wire.md`](Design/Plan/M0-the-wire.md), M0.1 to M0.4). **`GameCore`, `GameClient` and `GameLogic` are still shells**, holding a function that returns its own name so that every edge of the build — every reference, every include path, every link — is exercised by something rather than asserted. Each master include says to delete that placeholder when the library's first real declaration lands, and in the three above it is now owed.
