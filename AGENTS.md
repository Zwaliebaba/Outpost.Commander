...
The concrete layout is [`ADR-001`](Design/ADR/ADR-001-solution-layout.md) (2026-09-17): `OutpostCommander.slnx` at the root; eight projects — `Core`, `Content`, `Sim`, `Net`, `Replica` and `Client` as static libraries, `OutpostCommander` and `OutpostHost` as executables — each in a flat directory of its name, with `Tests/<Name>Tests` per library; the edges one way, as `Design/TechnicalDesign.md` §2 draws them; namespaces `Neuron` for the engine (`Core`, `Client`) and `Outpost` for the game. The constraints below are what that layout satisfies and what any change to it has to keep.
