...
**Five of the six libraries hold real code, and the sixth is the one that matters here.** ([`Design/Plan/M0-the-wire.md`](Design/Plan/M0-the-wire.md), M0.1 to M0.15.) `NeuronCore` has the byte codec, the packet header, fixed point, the vector, the sine table and the PRNG; `NeuronServer` the Winsock endpoint and the tick schedule; `NeuronClient` the `DatagramSocket` endpoint, the packet queue, the window metrics and fit transforms, the Direct3D 12 device and swap chain, the scene target and the scaled present that puts it on the glass, the interface pass that draws over it, and the gesture seam that is the only way in; `GameCore` the entity, the wire records and the command codec; `GameLogic` the world, the tick, the state hash, command intake and the host loop. **`GameClient` is the one still holding a function that returns its own name**, so that every edge of the build — every reference, every include path, every link — is exercised by something rather than asserted. Its master include says to delete that placeholder when the library's first real declaration lands, and that is now the only one outstanding.

**There is still no game in it**, and that is not a caveat: M0 is the wire, by design, because it is ordered by risk rather than by feature. Nothing below is a target to migrate towards — it describes the code as it must be written, and those three libraries are the first of it rather than an exception to it.
...
| R11, in prose and identifiers | [`Scripts/CheckSpelling.py`](Scripts/CheckSpelling.py), **run by hand, not gated** — see below |
...
Two executables over them: **`OutpostCommander`**, the game — a packaged UWP application whose view is a `CoreWindow`, built from `GameClient` and `NeuronClient`; and **`Server`**, the authoritative host — an ordinary Win32 console executable, built from `GameLogic` and `NeuronServer`. Each in a flat directory of its name, and one suite per library under `Tests/<Library>Tests`.
...
```
NeuronCore.h  ←  NeuronClient.h  ←  GameClient.h  ←  OutpostCommander/pch.h
       ↑              ↑                  ↑
       └── GameCore.h ┘                  │
       ↑                                 │
...
**`Scripts/` holds the checks, and the split in it is deliberate.** A **gate** is pass/fail, runs in CI and is named `Check*`: [`CheckProjectFiles.py`](Scripts/CheckProjectFiles.py) (§3's table), [`CheckDeterminism.py`](Scripts/CheckDeterminism.py) (R16), [`CheckDesign.py`](Scripts/CheckDesign.py) (figures and citations across `Design/`), [`CheckSuites.py`](Scripts/CheckSuites.py) (a suite that ran no test) and [`CheckHudGeometry.py`](Scripts/CheckHudGeometry.py) (the HUD's tiers, clear space and mirror, and the figures its `geometry.json` copies out of `Design/Interface.md` §1 — which `CheckDesign.py` does not reach). A **calculator** answers a question and has no verdict: [`DatagramBudget.py`](Scripts/DatagramBudget.py) costs a wire-format proposal. [`RunClangTidy.ps1`](Scripts/RunClangTidy.ps1) is neither yet — it reports (§1).

**[`CheckSpelling.py`](Scripts/CheckSpelling.py) is named `Check*` and is no longer a gate**, which makes it the one exception to the naming above. It was one until 2026-09-22 and §1 records why it stopped: minutes rather than a second, and mostly reporting generated build output. Fixing that would make it a gate again and the name would stop lying.
...
- [ ] The format check passes, and so do the gates: `python3 Scripts/CheckProjectFiles.py`, `Scripts/CheckDeterminism.py`, `Scripts/CheckDesign.py`, `Scripts/CheckHudGeometry.py`. They are seconds and they are what CI runs. **`CheckSpelling.py` is deliberately not on this list** (§1) — it takes minutes, it is not in CI, and the owner sweeps it occasionally; spell what you write correctly rather than running it every time.
