...
public:
  ...
  /// offset never recurs.
  ///
  /// **PLUS A TURN ABOUT AT EACH END, SINCE M1.17** (`OpenQuestions.md` Q59). A miner that fills its hold is
  /// facing the rock, with the station behind it, so it turns before it flies back, and turns again when it
  /// leaves the station. The turn is the tick's, pinned in `TickTests`, so here it is measured rather than
  /// restated. It must cost something, no more than a half turn at a Miner's rate, and the same at both ends.
  TEST_METHOD(ACycleTakesTheDesignsTicksAndTheNextStartsByItself)
  {
    ...
    const std::uint32_t turnAbout = firstUnload - full - 328u;
    const std::uint32_t halfTurnTicks =
      (32768u + Outpost::TurnAnglePerTick(Outpost::DesignId::Miner) - 1u) / Outpost::TurnAnglePerTick(Outpost::DesignId::Miner);
    Assert::IsTrue((turnAbout > 0u) && (turnAbout <= halfTurnTicks), L"the return leg, and the turn about that starts it");
    Assert::AreEqual(firstUnload + 39u, empty, L"forty ticks of unloading, the arrival tick among them");
    Assert::AreEqual(empty + turnAbout + 328u, secondExtract, L"the next cycle, with no order since the first");
    Assert::AreEqual(794u + (2u * turnAbout), secondExtract - firstExtract, L"a cycle");
