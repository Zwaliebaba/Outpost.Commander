...
public:
  ...
  TEST_METHOD(ACycleThroughTheLoopPaysItsOwner)
  {
    ...
    // One cycle and the first leg of the next: 795 ticks as `MiningTests` derives them, plus the two turns
    // about that M1.17 added, and room to spare -- fewer than a second cycle's unload, so still one hold.
