...
player block** — credits 4, last command applied 2, building design 1, progress 1, and since Q85 unlocks 1 and
research progress 1 — and nothing about
anyone else's; record count 1; removal count 1; fire count 1; and, **since `OpenQuestions.md` Q83 (2026-09-24)**, a
spent-rock count 1. **Twenty-four bytes** at any player count since `OpenQuestions.md` Q85 put two research bytes in
the player's own block, twenty-two before it and twenty-one before Q83. The mask behind the fire events
...
entity resets it to zero. The host takes the highest scores that fit — **97 records** at the pinned payload
with three removals, two fire events and a full spent-rock mask riding along — and sends them (98 before Q85, 99 before Q83). Relevance is an integer sum of:
