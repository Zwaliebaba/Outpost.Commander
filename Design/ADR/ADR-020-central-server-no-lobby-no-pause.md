...
The design surface is the opposite proportion: **50 `lobby` references and 21 `paus` references across `Design/` and `tasks/`.** This ADR and the commit that carries it change the normative ones — `GameDesign.md` §12, `Interface.md` §5, §6, §10 and §12, and `MatchSettings.h` — and leave the task plans' prose naming a lobby that the reader now knows is a configuration file.
