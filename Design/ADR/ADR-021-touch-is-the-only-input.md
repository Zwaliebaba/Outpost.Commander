...
**Still owed, and it is the owner's**: which device the game will be played on, and 7.5 mm on it in authored pixels, by finger on glass rather than by arithmetic on a specification. `t1-touch-interface/T6` records it after the owner's run. If that display's used width is 225 mm or more, 64 stands and nothing moves.
