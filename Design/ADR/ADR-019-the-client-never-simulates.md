...
**Two questions the topology opens and this ADR does not answer.** What operating system the host runs on: `OutpostHost` is MSVC and Windows only today, and the simulation's determinism survives a different toolchain (R16 forbids it floats) while the build system does not. And where the lobby lives: `MatchSettings` is "the lobby's values, fixed before the first tick", and with no local host to bootstrap from, something has to carry them to a host nobody is sitting at. Both are `m1-vertical-slice/N2`'s to answer now rather than M3's.
