> **Design reference, package v4.** Paths in this document refer to the original handoff folder. In this
> package: the OBJ set is `../Assets/Meshes/obj/`, the manifest `../Assets/Meshes/manifest.json`, the scripts
> `../Scripts/`, the UWP fragments `../Client/`, and the prompt `../INTEGRATION_PROMPT.md`.
> `CLAUDE_CODE_PROMPT.md` is superseded and not included.
...
| `Cruiser` | 180 Z | 2.8 : 1 | **hammerhead slab** — blunt wide prow, long parallel flanks, flared aft; four dorsal turrets | 316 | 948 | −14 … +21 |
...
| `Station` | 220 | — | **plus of four slim arms** under a floating habitat ring; tiered core, mast, docking cradles | 1,032 | 3,096 | −26 … +60 |
| `AsteroidA…E` | 62 / 82 / 111 / 128 / 167 | — | nine-sided, seven-section irregular lumps | 126 each | 378 each | symmetric about Y = 0 |

**Totals: 14 meshes, 3,522 triangles, 10,566 vertices.**
...
**The station is a plus, not a disc — and the habitat ring does not change that.** The ring is a 9-unit tube, about half an authored pixel at tactical zoom, so there it is a faint halo joining the arms and the plus still carries the read. At combat zoom it is the most distinctive thing on the map.

**Why a plus rather than a disc.** A disc at 12.9 px is a blob and reads as a very large asteroid.
...
plate, and cost one extra draw call per shape — 14 extra draws — for an effect invisible at 3.5 px and
marginal at 79 px.
...
| `Cruiser` | 316 | Sixteen-point plan — the most of any hull, and every point is a corner: hammer, waist, flank, aft flare. None of them rounds anything off, which is the test the twelve-point Scout failed. Five sections, four turret blisters (one per slot — a count that is legible at 158 px), two drive nacelles, an aft bridge. |
...
| `Station` | 1,032 | **Redesigned 2026-09-24** — the first version was a thick plate with a tower on it and read as less considered than the ships around it. Now: a tiered twelve-sided core (four-section drum, second tier, eight-sided command crown, antenna mast with crossbars, keel spire underneath); four slim chamfered arms, each ending in an **open docking cradle** — two prongs past the tip, so a ship visibly has somewhere to go — and a docking drum; a **habitat ring floating above the arms** on four diagonal spokes; two twin-barrel point-defence turrets on the E/W arms; paired radiator fins on the N/S arms. The same two rules as everything else: the plus outline stayed hard and few-pointed, and the ring is a 9-unit tube on a 72-unit radius, about 8:1 void to member, so it reads as a ring and not a disc. |
...
2,674 — 2.7×, before the Cruiser was added — and it bought a great deal at 53–193 px and **nothing whatsoever at 3.5 px.** The
...
the tangent and the UV are written as zeros, as the brief requires. 10,566 vertices × 52 = 536.6 KB, plus
20.6 KB of 16-bit indices, plus 3.0 KB of material and submesh blocks across 14 files (roughly 220 bytes
each). All byte figures in this handoff are KiB — 1,024 bytes — throughout.

> **≈ 560 KB total, of which ≈ 248 KB is zeros.**

A format with a packed vertex would carry the same geometry in about 310 KB — 28 live bytes per vertex
...
**Draw calls.** **14 instanced draws per frame** at full load — one per shape, covering every owner at
every player count:

`Scout` · `Frigate` · `Cruiser` · `ModuleFrame` · `ModuleShipyardL1` · `ModuleShipyardL2` · `ModuleOreProcessorL1` ·
...
decision and five are the asteroid variants. The `Cruiser`, added 2026-09-24, makes it 14.

**The Cruiser — authored 2026-09-24, on the owner's instruction.** This overrides the brief, which said
not to author it; the override is recorded here and in `meshes.json` (revision 3). **180 units long** —
the figure behind this handoff's earlier 10.5 px estimate. **Confirm it against the catalog before Q37
closes;** it is the one size in the bundle I did not echo from the brief. One file, ≈ 50 KB, one draw.

**The shape language had one quadrant left and the Cruiser took it:** long, blunt, massive. Scout is
blunt-and-wide, Frigate is long-and-pointed-and-cruciform, Station is radial arms; the Cruiser is a
**hammerhead slab** — a blunt 28-unit prow, a waist, long parallel flanks, a flared tail. **A fifth hull has
no quadrant left.** Whatever it is, it has to share an axis with one of these four and be separated by
something other than plan outline.

**I predicted it would be confused with the Station at tactical zoom. That prediction was wrong, and it
is worth saying so.** Both are ~11–13 px on their longest axis, but the Cruiser is 10.6 × 3.8 px and the
Station is 12.9 × 12.9 px. A 2.8 : 1 aspect against a 1 : 1 radial cross survives easily at that size, and
unlike the Scout–Frigate aspect difference it survives rotation, because 3.8 px of width is still under
half the length at a 45° heading. **The Cruiser is the only hull that clears the 3.5 px gate on geometry
alone** — which is the size doing the work, not the design.

**Consequence for the overlay (plate 7): the Cruiser gets no glyph, and I did not invent one.** Ring and
chevron were specified for miner and fighter because those two fail on geometry; the Cruiser does not.
Whether it should carry a glyph anyway, for consistency, is a question for the same register entry the
overlay already needs.
...
5. **The tree states the withdrawn rule — CORRECTED IN THE TREE, 2026-09-22, and this item was already
   stale when the handoff landed; re-amended when v4 landed on 2026-09-24.** This said that ADR-005,
   `TechnicalDesign.md` §7, `GameDesign.md` §10, `Design/README.md`, M1.9, M2.4 and M2.10b all still had
   meshes generated in code. That correction had already been made: the record is
   `ADR-005-a-mesh-is-a-cmo-file.md`, and there is nothing left to correct. Two figures in
   `TechnicalDesign.md` are now wrong for a second reason — the asteroid field's draw count (§8) and the
   module draw count (§7).
...
- **Whether a mesh may be authored off-origin.** The brief says the mesh origin is the simulated position;
  ...
  asymmetric in Z (the command block), and `Station`'s tiered core and mast put its volume centroid at Y ≈ +8. If anything asserts a centred centroid, those
  two fail.
