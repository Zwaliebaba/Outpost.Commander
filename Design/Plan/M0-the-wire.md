...
Q58), so neither the LAN run nor the wireless one will happen. ADR-008, ADR-003 and `TechnicalDesign.md`
...
Q58). Tap-to-visible is **76 ms on loopback** on the Surface Pro, and it stands as the answer: ADR-003's
...
host is simulating — **on the same machine**, since the owner's ruling of 2026-09-23 (Q58); a tap puts a
...
and M0.23 closed when the owner withdrew their two-machine runs (Q58). What they measured on one machine
