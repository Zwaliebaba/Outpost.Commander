...
| **M1.9b** | **Built, and not yet looked at.** The blackbody table is pinned at all eight stops and between them, the seeded field gives the same sky twice, and the ceiling is asserted rather than intended — 39 tests across `NeuronClientTests` and `GameClientTests`. **The asserted half found the design's own arithmetic wrong**: 1:3:9:27:81:243 sums to 364 and divides no round number of stars evenly, so the division three documents called exact was neither exact nor what the code computed — which truncated, and cost the second tier a star. The tiers are now 8, 25, 74, 223, 668 and 2,002, with a `CheckDesign` row behind them. **The cubemap bake and the draw are looked at rather than asserted and have not been**: nothing here has run on a device, so the band's seams, the dust lanes and whether 3,000 sprites read as a sky at the tactical zoom are all still open |
