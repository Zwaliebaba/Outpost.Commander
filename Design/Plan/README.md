...
**TWO STEPS WERE ADDED ON 2026-09-23 AND BOTH ARE BUILT, NOT RUN.** The owner asked for a way to load the
host from one machine, which became M1.14b's stress harness ([`ADR-022`](../ADR/ADR-022-a-bot-is-a-headless-client.md)),
and for a replication that keeps every datagram whole at a hundred players, which became M1.14c
([`ADR-024`](../ADR/ADR-024-replication-is-prioritized-records.md), with the player count of
[`ADR-023`](../ADR/ADR-023-the-player-count-is-configurable.md)). **Both compile and every suite passes on
`Debug|x64` in CI**, and that is the whole of what is known: no other pair has been built, and neither has
run against a real host. The table below says what each owes. *(2026-09-23, later: all four pairs now build
and pass on the Surface Pro, and the determinism hash held on each. Later again: **both have run against a
real `Server` on the Surface Pro**, from two seats to 128, with no tick abandoned. What M1.14c still owes
needs the packaged client and a finger.)*
...
has moved deliberately since**: at M2.6 and M2.7, below, and at M1.17, when ships began to turn and to
route around structures and each other. With both in the tree it is `0x073f1184808b252a`, the same on all four pairs.
Until it ran, every claim in that record about bit-identical behavior was an argument.
...
behind is not. M2.6 moved the pin to `0xc8f7f00e056d4d46` when the script began to mine, and M2.7
to `0x18e094912655348f` when its income became mined, both computed off Windows. M1.17 moved it on a
branch beside them, when ships began to turn and route. **With both merged it is `0x073f1184808b252a`, and that
value was the same on all four MSVC pairs**, which is also the run M2.7's pin was owed. This is the one thing `AGENTS.md` §6's CI scope guarantees nobody will notice: the
