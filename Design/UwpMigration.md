...
**3. The interface does not fit a finger. ANSWERED 2026-09-20, and the risk as written was a phantom.** It said that if a finger's smallest target did not fit the authored 1920×1080, what reopened was [`ADR-004`](ADR/ADR-004-renderer-foundation.md) — the authored resolution itself. `t1-touch-interface/T1` did the arithmetic (`Interface.md` §13) and the authored resolution **cancels out of it**: the required target is `14400 / width_mm`, so doubling the authored width doubles the target with it and the number of buttons on the screen is identical. `ADR-004` stands and could never have been the answer. What is real, and is now a *cost* rather than a risk, is the budget: `width_mm / 7.5` targets across, which is **29 on a 10.5-inch tablet against the 40-odd the mouse interface assumed**. §13 lists what fell out to pay for it — a tab strip in two panels, a third off the portrait grid, halved list densities — and every item is a count rather than a size. The one figure still owed is the owner's own device, which `T6` records.
