...
```
host loop (OutpostHost, or the host thread inside OutpostCommander), driven by wall time:
...
- **`Content`** loads the shipped tree, refuses each of a set of deliberately broken files with the right file and line, and applies a mod overlay; `OutpostHost --validate` is the same code and agrees by construction.
