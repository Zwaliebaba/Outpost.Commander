...
| [`Interface.md`](Interface.md) | What the operator sees and touches in M1: the frame at the authored resolution, the chrome and its palette, **the gestures and the 64-pixel touch target**, selection and orders, the five command tabs, the selection panel, the minimap and the readouts, the overlays | Design — **rewritten for touch 2026-09-20** by `t1-touch-interface/T1` under [`ADR-021`](ADR/ADR-021-touch-is-the-only-input.md) |
| [`UwpMigration.md`](UwpMigration.md) | The migration from Win32 to a packaged UWP application over a `CoreWindow`: the surface measured file by file, the tree before and after, the order of the work and why it is that order, and the risks ranked by what it costs to be wrong. The decisions it implements are ADR-013 to ADR-021 | Planned, not built |
...
| [`ADR/`](ADR/README.md) | Engineering decisions taken while building, one file per decision, numbered from `ADR-001` | ADR-001 to ADR-021 |
...
| the mouse, the keyboard, the hotkeys, aim mode | gone; a finger and a gesture ([`ADR-021`](ADR/ADR-021-touch-is-the-only-input.md)) |
