...
**The session is a match.** A match is one landscape, two to eight commanders — humans or AI in any mix — and a victory condition. It starts from a chosen base level (nothing but a builder and a command post; a small base; an established base), a power level and a technology level, all set in the **server's configuration** ([`ADR-020`](ADR/ADR-020-central-server-no-lobby-no-pause.md), 2026-09-20), and it ends when the condition is met or the players agree it has. Expected length is 30 minutes on a small landscape and two hours or more on a large one, which is long enough that a match must be **savable and resumable** by the host and **rejoinable** by a player who dropped (§10). **It is not pausable**: a central server has no player entitled to stop the world for the other seven, so rejoining is what a commander does instead of stopping it.
...
**In the match.** A player who drops is kept as a seat under AI control for a grace period and may rejoin, receiving their commander's view afresh; the match may be saved and resumed by the host later. **Nobody pauses** ([`ADR-020`](ADR/ADR-020-central-server-no-lobby-no-pause.md)). A long-running host that keeps a match open for weeks is the same machinery left running (owner, 2026-09-17: match-based, server-ready). Chat is text, to all or to allies.
...
| **M1 — Vertical slice** | Two commanders (one human, one scripted) on a Small landscape build extractors, generators, a factory and a lab, design a device and fight to annihilation, the client a replica from the first slice (owner, 2026-09-17). **Since [`ADR-019`](ADR/ADR-019-the-client-never-simulates.md) the two sides are two processes**: a player's M1 run is a client against an `OutpostHost`, and the in-process loopback survives only inside `OutpostCapture` and `Tests/IntegrationTests` | 1 landscape; 5 structures and one defence; 3 chassis; 3 drives; 4 modules (machine gun, cannon, mortar, builder); 30 research items; fixed-panel interface per `Interface.md`; the protocol and the replica over loopback; 1 scripted AI; the damage and visibility models complete; the M1 tables checked by the cost-efficiency script |
...
Revised on 2026-09-17 after the external review: Large landscapes, the dominance victory, mods and the content hash move to M3; M1 uses fixed panels; the look's extras come with M2. The review had also proposed deferring the protocol and the replica to M3, and the owner kept them in M1 (2026-09-17): replication is exercised over loopback from the first playable build.
