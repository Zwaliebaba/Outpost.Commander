...
| **M1** | **The fleet** | Two stations, the two designs, the current build item, move orders with ring assignment, selection by tap and by double tap. Two clients on one host. Host-side command validation. |
